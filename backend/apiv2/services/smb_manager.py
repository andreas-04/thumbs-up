...
class SMBManager:
    ...
    def __init__(
        self, storage_path: str, service_name: str = "ThumbsUp", port: int = None, workgroup: str = "WORKGROUP"
    ):
        """
...
"""
    ...
    def create_guest_user(self) -> bool:
        ...
        try:
            ...
            user_check = subprocess.run(["id", self.guest_user], capture_output=True, text=True)
            ...
            if user_check.returncode != 0:
                ...
                create_user = subprocess.run(
                    ["useradd", "-r", "-s", "/usr/sbin/nologin", self.guest_user], capture_output=True, text=True
                )

                if create_user.returncode != 0:
                    print(f"Warning: Could not create system user: {create_user.stderr}")
                    print("   Continuing anyway, Samba may work without system user...")
            ...
            stdout, stderr = process.communicate(input=f"{self.guest_password}\n{self.guest_password}\n")

            if process.returncode == 0:
                print(f"[OK] SMB user '{self.guest_user}' configured")

                # Enable the user
                subprocess.run(["smbpasswd", "-e", self.guest_user], check=False, capture_output=True)
                return True
            else:
                print(f"WARNING: Could not create SMB user: {stderr}")
                print("   System may not be configured correctly")
                return False
    ...
    def start_service(self) -> subprocess.Popen | None:
        """
        ...
        try:
            ...
            if process.poll() is None:
                ...
                print("ERROR: SMB service failed to start")
