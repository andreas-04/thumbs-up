...
class SecureNASClient:
    ...
    def setup_mtls(self):
        ...
        # Get PKI directory from environment variable set by main()
        pki_dir = Path(os.environ.get('THUMBSUP_PKI_DIR', '/app/pki'))
...
def main():
    ...
    # Priority: 1. Current directory 2. ~/.thumbsup/ 3. Package directory 4. Docker path
    cert_locations = [
        Path.cwd() / 'certs',  # Current directory
        Path.home() / '.thumbsup' / 'certs',  # User config directory
        Path(__file__).parent / 'certs',  # Package directory
        Path('/app/pki'),  # Docker path
        Path(__file__).parent.parent.parent.parent / 'backend' / 'pki',  # Development path
    ]
    ...
    for location in cert_locations:
        if location.exists() and all((location / f).exists() for f in required_certs):
            pki_dir = location
            logger.info(f"Using certificates from: {pki_dir}")
            break
    
    if not pki_dir:
        logger.error("Certificate files not found!")
        logger.error("Please ensure the following files exist in one of these locations:")
        logger.error(f"  - {cert_locations[0]}")
        logger.error(f"  - {cert_locations[1]}")
        logger.error("Required files: client_cert.pem, client_key.pem, server_cert.pem")
        logger.error("\nYou can obtain these from your ThumbsUp administrator.")
        sys.exit(1)
    
    # Update cert paths for SecureNASClient
    # Temporarily set PKI_DIR environment variable for client to use
    os.environ['THUMBSUP_PKI_DIR'] = str(pki_dir)
